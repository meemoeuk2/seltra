#include "memap.bmx.console.release.win32.x86.h"
...
struct BBDebugScope_12 _m_memap_fastintmap_scope ={
...
int _bb_main(){
	if (!_bb_main_inited) {
		...
		struct _m_memap_fastintmap_obj* volatile bbt_m=bbObjectNew(&_m_memap_fastintmap);
		BBINT bbt_i=1;
		BBINT bbt_t=bbMilliSecs();
		bbt_i=0;
		while(bbt_i<2999800){
			(bbt_m)->clas->m_insert_ii(bbt_m,((1*bbt_i)+brl_random_Rand(100,1)),(10*bbt_i));
			bbt_i=(bbt_i+1);
		}
